...
/**
 * 화면 버튼 표. **여기가 원본이다** — 버튼(index.html)도, 버튼 옮기기 편집 바도,
 * 첫 실행 조작 안내도 이걸 읽는다. 여러 군데 따로 적어두면 안내에 적힌 이름과
 * 버튼에 적힌 글자가 어긋난다 (실제로 편집 바는 「마이크」인데 버튼은 그림문자였다).
 *
 * `face` 버튼에 적히는 글자. 조작 안내도 이걸 쓴다 — 화면에서 그 글자를 찾아야 하니까
 * `name` 그 동작을 부르는 이름. 편집 바처럼 **버튼이 안 보이는 자리**에서 쓴다
 *
 * 일시정지만 둘이 다르다. 구석에 작게 두는 버튼이라 face 는 한 글자여야 하고,
 * 목록에서는 「일시정지」라고 불러야 알아본다.
 *
 * 그림문자(이모지)는 쓰지 않는다 — 폰트가 아니라 시스템 이모지로 그려져서 픽셀 화면과
 * 겉돈다. ⏸ 는 도트 폰트가 활자로 그려준다 (tests/emoji.test.js 가 선을 긋는다).
 */
export const ACTIONS = {
  left: { face: '◀', name: '왼쪽' },
  right: { face: '▶', name: '오른쪽' },
  jump: { face: '점프', name: '점프' },
  throw: { face: '마이크', name: '마이크' },
  dash: { face: '대시', name: '대시' },
  pause: { face: '⏸', name: '일시정지' },
};

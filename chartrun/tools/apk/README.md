...
> **먼저 — 대개는 이게 필요 없다.** `npm run build:web` 으로 구운 `docs/` 를
> GitHub Pages 에 올리면 링크 하나로 끝난다. 받는 쪽은 「홈 화면에 추가」만 누르면
> 아이콘이 생기고 전체화면으로 뜨고 인터넷 없이도 돌아간다 — 설치 경고도 없고
> 아이폰에서도 된다. 게임을 고쳐도 상대는 아무것도 안 해도 된다.
> ([../../README.md](../../README.md) 의 「남에게 주기」)
>
> 아래는 **진짜 앱으로 깔고 싶을 때**의 길이다.

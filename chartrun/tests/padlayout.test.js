...
import { clampSpot, marginsFor, layoutMode, sanitize } from '../src/ui/touchLayout.js';
...
test('저장된 값이 망가져 있어도 버티고 빈 자리로 돌아간다', () => {
  for (const junk of [null, undefined, 42, 'nope', [], { landscape: 'x' }]) {
    ...
    assert.deepEqual(out, { landscape: {}, rotated: {} });
  }
});
...
test('모르는 모드는 안 들인다', () => {
  ...
  assert.deepEqual(Object.keys(out).sort(), ['landscape', 'rotated']);
});
